...
class RendererFactoryTest extends TestCase
{
    ...
    /**
     * Test GET method with wrong AcceptPrivacy HTTP Header.
     */
    public function testGetWithWrongAcceptHttpHeader()
    {
        /** Mock request to return invalid AcceptPrivacy Types. */
        $this->_requestMock->expects($this->once())->method('getHeader')->willReturn('invalid');
        try {
            $this->_factory->get();
            $this->fail("Exception is expected to be raised");
        } catch (Exception $e) {
            $exceptionMessage = 'Server cannot match any of the given AcceptPrivacy HTTP header media type(s) ' .
